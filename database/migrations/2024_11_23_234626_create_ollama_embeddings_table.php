...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        Schema::create('ollama_embeddings', function (Blueprint $table) {
            $table->id();
            $table->morphs('embeddable');
            $table->string('embedding_model');
            $table->uuid('content_hash');
            $table->vector('vector', 768);
            $table->timestamps();
        ...
        DB::statement("CREATE INDEX ollama_embeddings_vector_idx ON ollama_embeddings USING hnsw (vector vector_cosine_ops)");
    ...
    {
        Schema::dropIfExists('ollama_embeddings');
    }
};
